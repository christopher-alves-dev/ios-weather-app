...
//  Weather App
//
//  Created by Christopher Alves on 19/06/24.
//
...
//        imageView.image = UIImage(named: "sun")
    ...
    private func fetchData() {
        service.fetchData(city: city) { [weak self] response in // para prevenir problemas de referências fortes (retain cycle)
    ...
    private func loadData() {
        ...
        temperatureLabel.text = forecastResponse?.current.temp
            .toCelsius()
        ...
        if forecastResponse?.current.dt.isDayTime() ?? true {
            ...
            statsStackView.backgroundColor = UIColor(named: "softBlue")
        } else {
            backgroundView.image = UIImage(named: "background-night")
            statsStackView.backgroundColor = UIColor(named: "softGray")
        }
    ...
    private func setupView() {
        ...
        setHierarchy() // necessário setar hierarquia antes das constraints para não dar erro. Não é possível setar constraints em para um componente que não está na hierarquia.
        setConstraints()
